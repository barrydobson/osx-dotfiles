function gpg-init
  if not begin
    ...
    [ -f $HOME/.gnupg/.gpg-agent-info ]
      and kill -0 (cut -d : -f 2 $HOME/.gnupg/.gpg-agent-info) ^/dev/null
    end
     # no, it is not running. Start it!
    killall gpg-agent
    gpg-agent --daemon --no-grab \
      --write-env-file $HOME/.gnupg/.gpg-agent-info >/dev/null ^&1
  end
  # get the agent info from the info file, and export it so GPG can see it.
  set -gx GPG_AGENT_INFO (cut -c 16- $HOME/.gnupg/.gpg-agent-info)
  set -gx GPG_TTY (tty)
end
